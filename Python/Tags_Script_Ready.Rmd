---
title: "Tegs_Script_Ready_Markdown"
...
```{r}
  ...
  generate_hmt_plots <-function(data){
    ...
    Defined_Grouping=c("fWB",
                       setdiff(unique(data$HMT_model),"fWB"))
    ...
    for (v in variables){
      ...
      #Goes through and creates a unique x-label depending on model and subject list
      ...
      #Caculates condition for significance asterisks
      ...
      p<-ggplot(GraphValues_gg,aes(x=x_label, y=.data[[v]]))+
        geom_boxplot(fill="lightgrey",outlier.shape=NA)+ #Boxplot
        stat_summary(fun.min=min,fun.max=max,geom="errorbar",width=0.5)+ #Range Bars
        geom_point(color="black",fill="lightgrey",shape=21)+ #Indvidual points
        facet_grid(~HMT_model,scales="free_x",space="free_x",switch="x",drop=FALSE)+
        labs(x=NULL,y=v)+ #Labels
        theme_classic()+
        theme(
          ...
          #Seperation lines for each unique models
